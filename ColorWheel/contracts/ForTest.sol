...
contract Test { //Wheel has 24 black 15 red 10 blue 1 gold
    ...
    function closeRound(uint256 _winNumber) external onlyOwner {
        require(rounds[currentRound].createdAt + timeToStart < block.timestamp, "You can not close wheel now");
        // (uint256 oldRange, bool hasSet) = VRF_V2.getRandomNumberByColorWheelRound(currentRound);
        ...
        unchecked {
            uint8 oldRange = uint8(_winNumber); //computate random number to an another range (0 to 49)
            uint16 newRange = uint16(oldRange);
            uint16 winNumber = newRange * 50 / 256;
